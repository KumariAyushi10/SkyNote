...
function changeBackgroundBasedOnWeather(weatherCode) {
  ...
  if (weatherCode === 0 || weatherCode === 1) {
    ...
    document.body.classList.add("stormy-weather");
  } else if (weatherCode >= 71 && weatherCode <= 86) {
    // Snow conditions
    document.body.classList.add("cloudy-weather");
  } else {
    // Default to clear weather for any other conditions
    document.body.classList.add("clear-weather");
  }
}
...
searchBox.addEventListener("submit", async (e) => {
  ...
  if (locationInput.value.trim() === "") {
    ...
    try {
      ...
      changeBackgroundBasedOnWeather(weather_code);
